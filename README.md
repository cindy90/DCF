...
- **State persistence** — 9-session build with full resume-after-interruption support
...
pip install tushare      # optional — A-share preferred data source (requires TUSHARE_TOKEN in .env)
pip install notebooklm   # optional — only if you use NotebookLM as a data source
```

Python 3.9+ required.

> **A-share users:** Set `TUSHARE_TOKEN` in your `.env` file. Get your token at [tushare.pro](https://tushare.pro/). When a tushare token is available, the skill automatically routes A-share tickers (.SH/.SZ) through tushare for CN GAAP native data.
...
The skill will ask you which data sources you have available, then guide you through the 9 sessions.
...
| **F** | DCF | WACC + UFCF + Terminal Value + EV→Equity bridge + sensitivity | 15–20 min |
| **G** | Sensitivity + Scenario | Independent sensitivity matrices + 3-scenario analysis | 15–20 min |
| **H** | Comps | 5-Block comparable company valuation analysis | 15–20 min |
| **J** | Valuation_Summary | DCF+Comps weighted valuation + range + analyst conclusion | 10–15 min |
...
| ✅ **A-share preferred** | **tushare API** (CN GAAP native) | `TUSHARE_TOKEN` in .env | Low | A-share auto-routed; IS/BS/CF + valuation metrics |
...
Yes. NotebookLM is optional. If you don't have it, the skill falls back to tushare (A-shares) or Web (Sina / Yahoo Finance) as the primary data source.

**Q: What if I only have annual data but the company reports quarterly?**
The skill auto-detects granularity using tushare (A-shares) or yfinance (HK/US). If quarterly data is available from any source, it uses quarterly. You can override with a manual answer during Phase 0.
...
Quarterly IS models produce 300+ lines of Python code per section. A single large script hits the LLM output cap and is silently truncated — the Excel file ends up partially written with no error. The 9-session, one-section-at-a-time approach prevents this entirely.
...
| Setup required | 9 sessions, ~2–3 hours total | Single session |
