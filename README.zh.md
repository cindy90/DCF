...
- **断点续跑** — 9 个 Session 独立构建，支持随时中断、下次接着跑
...
pip install tushare      # 可选 — A股首选数据源（需要在 .env 中设置 TUSHARE_TOKEN）
pip install notebooklm   # 可选 — 仅在使用 NotebookLM 作为数据源时需要
```

需要 Python 3.9+。

> **A股用户：**在 `.env` 文件中设置 `TUSHARE_TOKEN`。可在 [tushare.pro](https://tushare.pro/) 获取 Token。当 Token 可用时，Skill 自动将 A 股代码（.SH/.SZ）路由到 tushare 获取 CN GAAP 原生数据。
...
Skill 会先问你有哪些数据来源，然后引导你逐步完成 9 个 Session。
...
| **F** | DCF Valuation | WACC + UFCF + Terminal Value + EV→权益桥接 + 敏感性 | 15–20 分钟 |
| **G** | Sensitivity + Scenario | 独立敏感性全矩阵 + 三情景分析 | 15–20 分钟 |
| **H** | Comps（可比公司估值） | 5-Block 构建：基本信息/乘数统计/锚点/推算/桥接 | 15–20 分钟 |
| **J** | Valuation_Summary（综合估值） | DCF+Comps 加权 + 估值区间 + 分析师结论 | 10–15 分钟 |
...
| ✅ **A股首选** | **tushare API**（CN GAAP 原生数据） | 需 `TUSHARE_TOKEN` | 低 | A股自动路由；IS/BS/CF + 估值指标 |
...
可以。NotebookLM 是可选的。没有的话，Skill 自动降级到 tushare（A股）或 Web（Sina / Yahoo Finance）作为主要数据源。

**Q：我只有年度数据，但公司按季度披露，怎么处理？**
Skill 通过 tushare（A股）或 yfinance（港股/美股）自动识别颗粒度。只要任何来源显示有季度数据，就使用季度模式。Phase 0 时也可以手动确认。

**Q：Session 中途可以暂停、下次再继续吗？**
可以。每个代码块执行后都会在 Excel 的 `_State` Tab 写入进度标记。每个 Session 启动时会自动找到上次完成的步骤并从下一步继续。

**Q：为什么要跑 9 个 Session，不能一次跑完吗？**
季报模式的利润表每个 Section 就有 300+ 行 Python 代码。一次性生成会触及大模型的输出上限，代码被静默截断，Excel 文件写到一半没有任何报错。9 个 Session 逐段执行完全规避了这个问题。
...
| 配置成本 | 9 个 Session，约 2–3 小时 | 单 Session |
