...
```
DATA_SOURCES: [NLM, EXCEL, TUSHARE, WEB]   ← 实际用到的来源列表
DATA_CONFIDENCE: HIGH / MEDIUM / LOW
  HIGH   = NLM + EXCEL 同时可用（双源交叉验证）
  HIGH   = NLM/EXCEL + TUSHARE（API结构化数据可视为等效交叉源）
  MEDIUM = NLM 或 EXCEL 其中一个可用（单源 + Web cross-check）
  MEDIUM = 仅 TUSHARE（单 API 源，结构化但无独立交叉）
  LOW    = 仅 Web + PDF（兜底路径）
```
...
if is_a_share(ticker):
    try:
        ...
        if token:
            ...
            print(f"✅ A-share detected → tushare preferred (ts_code={ts_code})")
        else:
            print("⚠ A-share detected but TUSHARE_TOKEN not set → falling back to yfinance")
    except ImportError:
        print("⚠ A-share detected but tushare not installed → falling back to yfinance")
```

### 自动检测（tushare — A-share 首选）

```python
if USE_TUSHARE:
    # tushare 颗粒度检测：检查是否有季度报告
    try:
        income_df = pro.income(ts_code=ts_code, fields="ts_code,end_date,revenue")
        if income_df is not None and len(income_df) > 0:
            end_dates = income_df["end_date"].tolist()
            # 检查是否有非年报(0331/0630/0930结尾)
            quarterly_dates = [d for d in end_dates if not d.endswith("1231")]
            months = sorted(set(int(d[4:6]) for d in end_dates))
            print(f"tushare end_date months: {months}")
            if len(months) >= 3:
                detected = "Quarterly"
            elif set(months) == {6, 12}:
                detected = "Semi-annual"
            else:
                detected = "Annual"
        else:
            detected = "Unknown"
            print("tushare 检测失败: 无收入数据")
    except Exception as e:
        detected = "Unknown"
        print(f"tushare 检测失败: {e}")
    print(f"tushare 检测结果: {detected}")
```

### 自动检测（yfinance — HK/US 及 tushare 不可用时的备选）

```python
if not USE_TUSHARE or detected == "Unknown":
...
| 来源 | 检查方式 | 结果 |
|---|---|---|
| tushare（A-share）| 上方代码，看 end_date months 集合 | `{3,6,9,12}` → Quarterly |
| yfinance（HK/US）| 上方代码，看 months 集合 | `{3,6,9,12}` → Quarterly；`{6,12}` → Semi-annual |
...
**决策规则：**
```
四源中有任一显示 Quarterly → IS_GRANULARITY: Quarterly（不降级）
四源均显示 Semi-annual    → IS_GRANULARITY: Semi-annual
四源均显示 Annual         → IS_GRANULARITY: Annual
检测结果冲突              → 用 AskUserQuestion 问用户确认，不猜
```

如果 tushare + yfinance 均 FAIL 且无 NLM/Excel → 用交易所规则推断，然后用 AskUserQuestion 请用户确认。
...
⚠ **STOP 条件：** 如果颗粒度未能确认（tushare + yfinance 均 FAIL + 无其他来源 + 用户未确认），
...
### 2A — tushare 结构化数据（A-share 首选，USE_TUSHARE=True 时执行）

tushare 提供 CN GAAP 原生数据，比 Sina 网页抓取更稳定、更完整。A-share 优先使用 tushare，仅当 tushare 失败时降级到 Sina。
...
if USE_TUSHARE:
    ...
    # ── 完整三表提取（CN GAAP 原生，单位：元）──
    ts_data = {}

    # 利润表（合并报表，最近5年年报 + 最近4个季度）
    try:
        income_annual = pro.income(ts_code=ts_code, report_type='1')
        if income_annual is not None:
            annual = income_annual[income_annual["end_date"].str.endswith("1231")].head(5)
            ts_data["income_annual"] = annual
            print(f"  ✅ tushare IS (annual): {len(annual)} years")
            # 季度数据（如需要）
            quarterly = income_annual[~income_annual["end_date"].str.endswith("1231")].head(8)
            ts_data["income_quarterly"] = quarterly
            print(f"  ✅ tushare IS (quarterly): {len(quarterly)} quarters")
    except Exception as e:
        print(f"  ⚠ tushare IS failed: {e}")

    # 资产负债表
    try:
        bs_df = pro.balancesheet(ts_code=ts_code, report_type='1')
        if bs_df is not None:
            bs_annual = bs_df[bs_df["end_date"].str.endswith("1231")].head(5)
            ts_data["balance_annual"] = bs_annual
            print(f"  ✅ tushare BS: {len(bs_annual)} years")
    except Exception as e:
        print(f"  ⚠ tushare BS failed: {e}")

    # 现金流量表
    try:
        cf_df = pro.cashflow(ts_code=ts_code, report_type='1')
        if cf_df is not None:
            cf_annual = cf_df[cf_df["end_date"].str.endswith("1231")].head(5)
            ts_data["cashflow_annual"] = cf_annual
            print(f"  ✅ tushare CF: {len(cf_annual)} years")
    except Exception as e:
        print(f"  ⚠ tushare CF failed: {e}")

    # 财务指标（ROE、毛利率、净利率、EPS 等）
    try:
        indicator = pro.fina_indicator(ts_code=ts_code)
        if indicator is not None:
            ind_annual = indicator[indicator["end_date"].str.endswith("1231")].head(5)
            ts_data["indicator"] = ind_annual
            print(f"  ✅ tushare indicators: {len(ind_annual)} years")
    except Exception as e:
        print(f"  ⚠ tushare indicators failed: {e}")

    # 每日估值指标（最新 PE/PB/PS/市值）
    try:
        daily_basic = pro.daily_basic(ts_code=ts_code, fields="ts_code,trade_date,total_mv,pe_ttm,pb,ps_ttm")
        if daily_basic is not None and len(daily_basic) > 0:
            latest = daily_basic.iloc[0]
            ts_data["valuation"] = {
                "market_cap_yi": round(latest["total_mv"] / 10000, 2) if latest.get("total_mv") else None,
                "pe_ttm": round(latest["pe_ttm"], 1) if latest.get("pe_ttm") else None,
                "pb": round(latest["pb"], 2) if latest.get("pb") else None,
                "ps_ttm": round(latest["ps_ttm"], 1) if latest.get("ps_ttm") else None,
                "trade_date": latest.get("trade_date", ""),
            }
            print(f"  ✅ tushare valuation: PE={ts_data['valuation']['pe_ttm']}, PB={ts_data['valuation']['pb']}")
    except Exception as e:
        print(f"  ⚠ tushare valuation failed: {e}")

    # 分红数据
    try:
        dividend = pro.dividend(ts_code=ts_code)
        if dividend is not None and len(dividend) > 0:
            ts_data["dividend"] = dividend.head(5)
            print(f"  ✅ tushare dividend: {len(ts_data['dividend'])} records")
    except Exception as e:
        print(f"  ⚠ tushare dividend failed: {e}")

    # ── 单位换算提醒 ──
    # tushare 数据单位为「元」（RMB），写入 Raw_Info 时需统一到模型单位：
    #   万元: /1e4   百万元: /1e6   亿元: /1e8
    print(f"\n⚠ tushare 数据单位为「元」，写入 Raw_Info 时需按 UNIT 换算")
    print(f"  模型 UNIT: {state.get('UNIT', '待确认')}")
```

**tushare 关键字段映射（利润表 → Raw_Info）：**
```
tushare field          → Raw_Info label                          → 说明
revenue                → IS: Revenue (Total)                     → 营业总收入
oper_cost              → IS: COGS                                → 营业成本
operate_profit         → IS: 营业利润 (Operating Profit)         → R8 plug 的基准
total_profit           → IS: PBT                                 → 利润总额
income_tax             → IS: Income Tax                          → 所得税费用
n_income               → IS: Net Income                          → 净利润
n_income_attr_p        → IS: Attr to Owners                      → 归母净利润
minority_gain          → IS: Attr to NCI                         → 少数股东损益
sell_exp               → IS: Selling Expense                     → 销售费用
admin_exp              → IS: Admin Expense                       → 管理费用
rd_exp                 → IS: R&D Expense                         → 研发费用
fin_exp                → IS: Finance Cost                        → 财务费用
```

**tushare 关键字段映射（资产负债表 → Raw_Info）：**
```
money_cap              → BS: Cash                                → 货币资金
accounts_receiv        → BS: AR                                  → 应收账款
inventories            → BS: Inventory                           → 存货
total_cur_assets       → BS: Total CA                            → 流动资产合计
fix_assets             → BS: PP&E net                            → 固定资产
total_nca              → BS: Total NCA                           → 非流动资产合计
total_assets           → BS: Total Assets                        → 资产总计
lt_borrows             → BS: LT Borrowings                       → 长期借款
total_liab             → BS: Total Liabilities                   → 负债合计
total_hldr_eqy_exc_min_int → BS: Equity (Owners)                → 归属母公司所有者权益
minority_int           → BS: NCI                                 → 少数股东权益
```

**tushare 关键字段映射（现金流量表 → Raw_Info）：**
```
n_cashflow_act         → CF: CFO                                → 经营活动现金流净额
n_cashflow_inv_act     → CF: CFI                                → 投资活动现金流净额
n_cashflow_fnc_act     → CF: CFF                                → 筹资活动现金流净额
c_cash_equ_end_period  → CF: Ending Cash                        → 期末现金及现金等价物
```

### 2B — Web Cross-Check / Fallback（始终运行）

**角色 1 — 核心数字验证（NLM 或 Excel 或 tushare 可用时）**
快速抓取 Sina/Yahoo 最近 2 年的 Revenue / Net Income / Total Assets / Cash，
与 NLM/Excel/tushare 数字对比：
- 差异 < 5% → 确认，标注 ✅
...
A-share 路径（USE_TUSHARE=True）：直接使用上方 Step 2A 提取的 tushare 三表数据。
tushare 提供完整 CN GAAP 三表，无需网页抓取。只需做单位换算。

非 A-share / tushare 不可用时：完整执行 Sina/Yahoo 抓取：
...
| 报表 | tushare (A-share) | Sina | Yahoo | 通常选 |
|---|---|---|---|---|
| IS | ~50+字段，CN GAAP原生，API直取 | ~30行，CN GAAP原生 | ~20行，可能IFRS重分类 | tushare (A-share) / Sina (备选) |
| BS | ~80+字段，完整拆分 | ~94行，完整拆分 | ~30-50行 | tushare (A-share) / Sina (备选) |
| CF | ~50+字段+补充D&A | ~40行+补充D&A | ~25行，无补充资料 | tushare (A-share) / Sina (备选) |

单位换算：tushare=元(÷1e4→万元)；Sina=万元(×10→千元)；招股书/Yahoo=千元(×1)。
...
```
财务数字：  Excel（用户直接提供，已验证）≥ NLM（原始文件提取）> tushare（API结构化数据）> Web（Sina网页抓取/重分类）
定性情报：  NLM（原文引用）> Web Search > 人工读PDF
估值指标：  tushare（A-share CN GAAP原生）> yfinance（IFRS/US GAAP）
```
...
完成后写入 _State：
```
DATA_SOURCES: [NLM, EXCEL, TUSHARE, WEB]   ← 实际用到的列表，如仅TUSHARE则 [TUSHARE]
...
```
SKILL_VERSION: 4.7
DATA_SOURCES: [NLM, EXCEL, TUSHARE, WEB]
DATA_CONFIDENCE: HIGH / MEDIUM / LOW
