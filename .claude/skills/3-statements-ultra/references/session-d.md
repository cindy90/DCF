...
```
  ...
  c. Write CFF rows:
       Historical formulas:
         ST Borrowing Chg:  first yr f"=BS!{cl}{st_row}"  |  other yrs f"=BS!{cl}{st_row}-BS!{prev_cl}{st_row}"
         LT Payable Chg:    first yr f"=BS!{cl}{lt_row}"  |  other yrs f"=BS!{cl}{lt_row}-BS!{prev_cl}{lt_row}"
         Lease Principal:   first yr f"=-BS!{cl}{lease_row}"  |  other yrs f"=-(BS!{cl}{lease_row}-BS!{prev_cl}{lease_row})"
         Dividends:         0 (historical) | forecast: f"=-IS!{cl}{ni_row}*Assumptions!{div_pct_cell}"
         Other CFF (PLUG):  f"=Raw_Info!{col}{raw_map['CF: CFF Net']}-{cl}{st_chg_row}-{cl}{lt_chg_row}-{cl}{lease_row}-{cl}{div_row}"
                            ↑ Forces CFF Total = Source CFF Net — same principle as R3 for CFO.
                            ❌ NEVER use f"=Raw_Info!{col}{BorrowProceeds}-Raw_Info!{col}{DebtRepay}" — double-counts vs BS deltas!
       Forecast formulas:
         ST/LT/Lease: BS deltas (same as above)
         Dividends: f"=-IS!{cl}{ni_row}*Assumptions!{div_pct_cell}"
         Other CFF: 0 (or explicit equity issuance if modeled)
         CFF Total: =SUM(st:other_cff)
  ...
  e. Write Net Change, Beginning Cash, Ending Cash:
       ...
       Beg_Cash all other years: f"=BS!{prev_cl}{cash_row}"  ← prior year BS Cash (含受限)
  f. Write Restricted Cash Adj (R5c):
       Historical: f"=Raw_Info!{cl}{raw_bs_cash_row}-{cl}{end_cash_row}"
       Forecast:   0
  f2. Write Total Cash (BS linkage): f"={cl}{end_cash_row}+{cl}{restrict_adj_row}"
  f3. Write CF CHECK: f"={cl}{total_cash_row}-BS!{cl}{cash_row}"
  g. IMMEDIATELY back-fill BS Cash for this year:
       ws_bs.cell(cash_row, this_col).value = f"=CF!{this_cl}{total_cash_row}"
  h. Verify back-fill:
...
```
KEY_CELLS_CF: {"EndCash_row": N, "RestrictedCashAdj_row": N, "TotalCash_row": N, "CFO_row": N, "Others_row": N, "CF_CHECK_row": N, "wc_start_row": N, "wc_end_row": N, "cfi_row": N, "cff_row": N}
```
