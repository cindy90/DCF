...
Returns tab follows **WIND-style 7-section layout** (87 rows, ~60 metrics). All cells reference IS/BS/CF by row number from KEY_CELLS — no hardcodes.

```
一、盈利能力  (R5-R18):   毛利率, EBITDA/Margin, EBIT/PBT/净利率, ROA, ROE, ROIC,
                          销售/管理/研发/期间费用率, 有效税率
二、成长指标  (R21-R27):  Revenue/GP/EBIT/NI/TA/Equity/CFO 同比增长率
三、运营效率  (R30-R39):  AR/Inv/AP Days, CCC, AR/Inv/TA/PPE周转率, NWC, NWC/Rev
四、杠杆与偿债  (R42-R52):  资产负债率, 产权比率, 净负债率,
                          有息负债/EBITDA (净), 流动/速动/现金比率, 利息保障年限
五、现金流质量  (R55-R64):  CFO, Capex, FCF, FCF Margin, 净利含金量, CFO/TA, Capex/D&A
六、杜邦五因素  (R67-R72):  Tax Burden × Interest Burden × EBIT Margin
                          × Asset Turnover × Equity Multiplier = DuPont ROE
七、绝对值参考  (R75-R87):  Revenue/GP/EBITDA/EBIT/NI/归母净利和等主要P&L绝对量
```

ROIC = NOPAT / AVERAGE(IC_curr, IC_prior);  IC = NWC + Net LT Op Assets
ROE  = Attr_Owners NI / Avg Equity_Owners
ROA  = NI / Avg Total Assets
DuPont 5-factor: `NI/PBT × PBT/EBIT × EBIT/Rev × Rev/AvgTA × AvgTA/AvgEq`

First historical year: use single-period balance for ratio denominators (no prior-year avg).
Safe-guards: `IF(denom=0, 0, ...)` on all ratio cells; growth rates use `IF(prior=0,"NA",...)`.
...
**数据验证层 (run after model is complete):**

根据 `USE_TUSHARE` 标志选择验证数据源。A-share 优先使用 tushare（CN GAAP 原生），HK/US 使用 yfinance。

```python
# ────── A-share: tushare 验证 (首选) ──────
if USE_TUSHARE:
    import tushare as ts
    pro = ts.pro_api()
    ts_code = to_tushare_code(ticker)  # e.g. "600519.SH"

    # 1. Revenue & Net Income magnitude check
    df_inc = pro.income(ts_code=ts_code, report_type='1',
                        fields='end_date,revenue,n_income_attr_p')
    df_inc = df_inc[df_inc['end_date'].str.endswith('1231')]  # 年报
    for _, row in df_inc.iterrows():
        year = row['end_date'][:4]
        print(f"tushare {year}: Revenue={row['revenue']:,.0f}元, NI={row['n_income_attr_p']:,.0f}元")
    # Flag if gap vs model > 10%; tushare unit=元, convert to model unit

    # 2. Valuation sanity check (PE/PB/PS)
    df_basic = pro.daily_basic(ts_code=ts_code, fields='trade_date,pe_ttm,pb,ps_ttm,total_mv')
    latest = df_basic.iloc[0] if len(df_basic) > 0 else None
    if latest is not None:
        print(f"tushare Latest: PE_TTM={latest['pe_ttm']:.1f}, PB={latest['pb']:.2f}, PS_TTM={latest['ps_ttm']:.1f}")
        print(f"  Total MV={latest['total_mv']/1e4:.1f}亿元")

    # 3. Key ratios cross-check (ROE, margins)
    df_fi = pro.fina_indicator(ts_code=ts_code, fields='end_date,roe_dt,grossprofit_margin,netprofit_margin')
    df_fi_annual = df_fi[df_fi['end_date'].str.endswith('1231')]
    for _, row in df_fi_annual.head(3).iterrows():
        print(f"tushare {row['end_date'][:4]}: ROE_dt={row['roe_dt']:.1f}%, "
              f"Gross={row['grossprofit_margin']:.1f}%, Net={row['netprofit_margin']:.1f}%")

# ────── HK/US 及 tushare 不可用时: yfinance 验证 ──────
else:
    import yfinance as yf
    tk = yf.Ticker(ticker)   # e.g. "BABA", "PDD", "0700.HK"
    # ⚠ yfinance returns IFRS-reformatted data — use only for magnitude sanity checks
    # ⚠ A-share EPS from yfinance is USD-denominated → DO NOT use for PE calculation
