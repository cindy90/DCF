...
pip install tushare      # recommended for A-share (沪深) — requires TUSHARE_TOKEN env var
...
- ~1–2 hours across 9 sessions (each session is independent — pause and resume anytime)
...
| **tushare API** (A-share structured data) | `TUSHARE_TOKEN` env var | Very low | ✅ A-share preferred |
...
SESSION F  — DCF Valuation (WACC + FCF + TV + 敏感性)   (~15–20 min)
SESSION G  — Sensitivity + Scenario tabs               (~15–20 min)
SESSION H  — Comps (可比公司估值分析)                    (~15–20 min)
SESSION J  — Valuation Summary (综合估值汇总)           (~10–15 min)
```
...
## 🇨🇳 A-Share Data Routing — tushare vs yfinance

**When the ticker is an A-share (.SH / .SZ / pure 6-digit code), prefer tushare over yfinance.**
yfinance normalizes A-share data to IFRS/US GAAP format and returns USD-denominated EPS — both introduce systematic errors for CN GAAP models.
...
def is_a_share(ticker: str) -> bool:
    """Detect if ticker is an A-share stock."""
    t = ticker.strip().upper()
    if t.endswith('.SS') or t.endswith('.SH') or t.endswith('.SZ'):
        return True
...
def to_tushare_code(ticker: str) -> str:
    ...
    t = ticker.strip().upper()
    if t.endswith('.SS'):
        return t.replace('.SS', '.SH')   # yfinance 600519.SS → tushare 600519.SH
...
def tushare_available() -> bool:
    """Check if tushare is installed and token is set."""
    try:
...
**Routing rule (apply in Phase 0 and Cross_Check):**
```
if is_a_share(ticker) and tushare_available():
    → Use tushare for: granularity detection, 0A financial data, Cross_Check validation
    → Fallback to yfinance only if tushare API call fails
else:
    → Use yfinance (HK / US / ADR tickers)
```

**tushare data notes:**
- Unit: 元 (元, NOT 万元). Divide by 1e4 → 万元, or 1e8 → 亿元
- `pro.income()` returns cumulative YTD for quarterly; same YTD→Q conversion as Sina
- `pro.balancesheet()` is point-in-time (no YTD conversion needed)
- Filter `report_type='1'` to get consolidated (合并) reports, not parent-only
- Use `end_date` patterns: `*1231` = annual, `*0331/0630/0930` = quarterly
- `pro.fina_indicator()` provides ROE, margins, EPS in RMB — use instead of yfinance for A-shares
...
The model is built across **9 sessions**. Each session starts with a clean context, loads
...
SESSION F   — DCF Valuation tab                → Read references/session-f.md
               └─ WACC + UFCF + Terminal Value + EV→Equity + Sensitivity Tables
SESSION G   — Sensitivity + Scenario tabs      → Read references/session-g.md
               └─ 独立敏感性全矩阵 (EV + 权益价值) + 三情景P&L/FCF/估值汇总
SESSION H   — Comps tab (可比公司估值分析)      → Read references/session-h.md
               └─ 5-Block layout: 基本信息/乘数统计/锚点/推算/桥接
SESSION J   — Valuation_Summary tab            → Read references/session-j.md
               └─ 综合估值汇总 (DCF+Comps 加权 + 估值区间 + 分析师结论)
```
...
```
[1]  Summary            ← built in E; all figures link to model cells
[2]  Assumptions        ← every forecast driver; single source of truth
[3]  IS                 ← Income Statement
[4]  BS                 ← Balance Sheet
[5]  CF                 ← Cash Flow Statement (indirect)
[6]  Returns            ← WIND-style 7-section: 盈利/成长/运营/杠杆/现金流质量/DuPont/绝对值
[7]  DCF                ← WACC + UFCF + Terminal Value + EV→权益桥接
[8]  Sensitivity        ← DCF敏感性全矩阵: 表A-D (EV+权益价值 × WACC×g/倍数) + 表E Tornado
[9]  Scenario           ← 三情景分析 (悲观/中性/乐观): 假设矩阵 + 财务摘要 + DCF估值汇总
[10] Comps              ← 可比公司估值分析 (5 Blocks: 基本信息/乘数/锚点/推算/桥接)
[11] Valuation_Summary  ← 综合估值汇总 (DCF+Comps 加权 + 估值区间 + 分析师结论)
[12] Cross_Check        ← assumption validation + revision log
[13] Raw_Info           ← source extraction (built first; never re-read source after)
...
project_config.yaml    ← project-specific configuration (periods, peers, anchors, weights)
_comps_data.json       ← tushare cached comparable company data
```

---

## ██ NEW PROJECT SETUP PROTOCOL ██
## ██ Run this ONCE when starting a model for a new company. ██

**Every new company gets its own subdirectory under `DCF agent/`.** The directory contains:
- `project_config.yaml` — project-specific configuration (copied from template)
- `{company}_3statement_model.xlsx` — the model file (created by SESSION A)
- `_comps_data.json` — cached comparable company data (created by SESSION H)
- `_model_log.md` — checkpoint log (created during build)
- `_pending_links.json` — deferred cross-sheet references
- `session-h.md` — project-specific Comps session notes (optional, for special cases)

### Setup Steps

```
Step 1: Create project directory
        mkdir "DCF agent/{公司简称}"

Step 2: Copy config template
        cp "DCF agent/project_config_template.yaml" → "DCF agent/{公司简称}/project_config.yaml"

Step 3: Fill ALL "TODO:" fields in project_config.yaml
        - identity: company name, business description
        - paths: model file name
        - periods: historical + forecast years
        - comps: core peers, excluded tickers, industry index
        - anchor_refs: IS/BS row numbers (can be updated after SESSION B/C)
        - dcf_cells: DCF tab cell addresses (updated after SESSION F)
        - valuation: method weights, current round value
        - summary: catalysts, risks
        - analyst_notes: commentary text

Step 4: Proceed to SESSION A with the config filled
```

**Key rule:** `project_config.yaml` is the SINGLE SOURCE of project-specific parameters.
Build scripts MUST read from this config — never hardcode company names, ticker lists,
row numbers, or file paths inside the Python script itself.

---

## ██ SHARED MODULE PROTOCOL ██
## ██ All build scripts import from shared/. No inline style definitions. ██

The `DCF agent/shared/` directory contains reusable modules:

```
shared/
  __init__.py        — re-exports all public APIs
  style_utils.py     — color palette, font/fill/alignment/border factory functions, number formats
  excel_helpers.py   — write_blk_hdr, write_col_hdrs, write_cell, make_header_row, add_section_row
  data_utils.py      — nv(), stat(), exch(), load_json(), filter_positive()
  state_io.py        — read_state(), write_state_key(), read_key_cells(), get_key_row()
  config_loader.py   — load_project_config(), ProjectConfig class
```

### Mandatory Import Pattern

Every build script (build_e_block1.py, build_comps_sheet.py, build_valuation_summary.py, etc.)
MUST import from `shared/` instead of defining helpers inline:
...
1. **NEVER define `fill()`, `fnt()`, `nv()`, `stat()`, color constants, or border helpers inside a build script** — import them from `shared/`
2. **If a utility function does not exist in `shared/`, add it to the appropriate `shared/*.py` module** — do not create project-local helpers
3. **`shared/` is the canonical style definition** — all projects share identical visual formatting
4. **Project-specific data** (peer lists, row numbers, weights) goes in `project_config.yaml`, NOT in `shared/`

### ⚠️ Historical Note

Some legacy per-project build scripts (e.g. `build_comps_sheet.py`, `build_valuation_summary.py`)
were written BEFORE `shared/` was extracted. They still contain inline style/helper definitions —
these are **legacy code and should NOT be copied**. When creating scripts for a new project:
- ✅ Reference legacy scripts for **business logic** (Block layout, data processing flow)
- ❌ Do NOT copy their inline `fill()`, `fnt()`, `nv()`, color constants, or `read_state()` definitions
- ✅ Import everything from `shared/` instead
...
**R5b — CFF plug for historical years (prevents double-counting):**
CFF rows model individual items from BS deltas (ST Borrowing Δ, LT Payable Δ, Lease Principal, Dividends).
These BS deltas already capture the NET effect of all borrowing/repayment activity.
`Other CFF` must NOT re-decompose source CF items (e.g., `=Raw_Info!{col}{Borrowing Proceeds} - Raw_Info!{col}{Debt Repayment}`) — this double-counts against BS deltas.

Correct formula for historical years:
```
CF!Other_CFF = Raw_Info!{col}{CFF_Net} - {col}{ST_chg} - {col}{LT_chg} - {col}{Lease} - {col}{Div}
```
This forces `CFF Total = Source CFF Net` (same principle as R3 forcing CFO = Source CFO).
Forecast years: `Other CFF = 0` (or explicit equity issuance from Assumptions if applicable).

**R5c — Restricted Cash Reconciliation (CN GAAP 受限货币资金调整):**
CN GAAP BS "货币资金" includes restricted cash; CF "现金及现金等价物" excludes it.
Add reconciliation rows after CF Ending Cash to keep all three statements fully linked:
```
CF!R_EndCash:      = Beg + Net Change              (pure CF flow)
CF!R_RestrictAdj:  = Raw_Info!BS_Cash - R_EndCash   (historical) / 0 (forecast)
CF!R_TotalCash:    = R_EndCash + R_RestrictAdj      (= source BS Cash)
CF!R_CHECK:        = R_TotalCash - BS!Cash          (must = 0)
BS!Cash:           = CF!R_TotalCash                 (all years, uniform)
CF!Beg_Cash(yr+1): = BS!Cash(yr)                    (rolling forward, includes restricted)
```
If source BS Cash = CF End Cash (no restricted cash), the adjustment row = 0 — no impact.

**R6 — Cash last:** All BS items built → CF complete year by year → `BS!Cash = CF!TotalCash` back-filled immediately after each year's CF is complete.
...
**R9 — Five reconciliation checks (all must = 0):**
...
- `CFF CHECK = Model CFF Total − Source CFF Net` (historical years only; tolerance ±0.01)
...
8b. **CFF Other decomposing source CF instead of using plug** — `=Raw_Info!BorrowProceeds - Raw_Info!DebtRepay` double-counts against BS-delta CFF rows; use R5b CFF plug: `Other CFF = Source CFF Net − modeled CFF items`
...
21. **Defaulting to annual when quarterly data exists** — check R10 first; run yfinance/tushare granularity auto-detect (A-share: prefer tushare `pro.income()` to check quarterly availability); `{3,6,9,12}` = quarterly, `{6,12}` = semi-annual; do NOT use yfinance/tushare as primary data source
22. **Using yfinance forwardEps for A-share/HK forecast EPS** — yfinance returns USD-denominated EPS for A-shares (distorts CNY PE by ~6×); for A-share tickers use **tushare** (`pro.fina_indicator()` for ROE/margins, `pro.daily_basic()` for PE/PB/PS) — it returns CN GAAP native data in RMB; US/ADR tickers may use yfinance directly
...
**Scope:** SESSION F only. DCF tab reads exclusively from IS/BS/CF/Assumptions — never from Returns or Cross_Check.

- WACC inputs (Rf, ERP, βU, Kd, tax rate) go in **Assumptions tab** OR as dedicated input rows in the DCF tab with light-blue fill — they are model inputs, NOT hardcodes inside formula strings.
- FCF bridge: `UFCF = NOPAT + D&A + Capex(CF) − ΔNWC`. All components must link to named rows in IS/BS/CF (use KEY_CELLS from _State). No hardcoded cash flows.
- Terminal value: always compute **two methods** — Gordon Growth (`TV = UFCF_{T+1}/(WACC−g)`) and EV/EBITDA exit multiple — then cross-check.
- Sensitivity tables must recompute EV inline (not reference a single WACC cell); each cell in the grid has its own independent formula.
- Net Debt for bridge = `IBD − Cash` using **FY_LAST_HIST** balance sheet col (not forecast); reference `KEY_CELLS_BS` from _State.
- `g` (terminal growth) must satisfy `g < WACC`; use `IF((WACC−g)>0, …, "ERR: g≥WACC")` guard.
- DCF tab color: green (#70AD47). Tab order: after Returns, before Cross_Check (or appended after Summary).

---

## R13 — Sensitivity & Scenario Tab Rules

**Scope:** SESSION G only. Both tabs are **read-only outputs** — they never modify IS/BS/CF/Assumptions.

### Sensitivity tab rules
- Tab color: purple (#7030A0). Inserted after DCF, before Cross_Check.
- Must contain **4 grid tables**: EV Gordon (WACC×g), EV Exit (WACC×多倍数), Equity Value Gordon, Equity Value Exit — plus a Tornado table.
- Every grid cell uses a **fully independent inline formula**: hardcoded w/g/m for that cell only; references `DCF!B33:F33` (UFCF), `DCF!B48` (terminal EBITDA), `DCF!B70` (Net Debt). No cell references another sensitivity cell.
- Base-case cell (WACC=11%, g=3%, 10×) highlighted green. All other cells white.
- Net Debt deducted for Equity Value tables: `EV_inline − DCF!B70`.
- `g < WACC` guard: `=IF((w−g)>0, formula, "ERR")`.

### Scenario tab rules
- Tab color: orange (#ED7D31). Inserted after Sensitivity.
- **Block 1** (Assumption Inputs): Bear / Base / Bull columns.
  - **Base column: MUST link to `Assumptions!B{row}` using ASM_MAP from _State — no hardcoded Base values.**
  - Bear/Bull columns: analyst input deltas; formulas like `=Assumptions!B{row}+({delta})` or `=MAX(0,Assumptions!B{row}−{delta})`.
- **Block 2** (Financial Summary): 5 forecast years × 3 scenarios. Base columns = live `IS!/CF!/DCF!` cell references. Bear/Bull columns = `Base_cell × scale_factor` (P&L items) or `Base_pct_cell + delta` (margin items).
- **Block 3** (Valuation): EV Gordon / Exit / Equity Value for 3 scenarios. Base = `DCF!B{row}` direct links. Bear/Bull = UFCF-scaled inline Gordon/Exit formulas using `DCF!B21` (WACC), `DCF!B41` (g), `DCF!B47` (Exit multiple), `DCF!B70` (Net Debt) — all referenced, not hardcoded.
- IS/BS/CF tabs are **never modified** by this sheet. Scenario outputs are computed entirely inline within the Scenario tab.
- WACC, g, Exit multiple, Net Debt are always referenced from DCF tab cells, not typed as numbers.
