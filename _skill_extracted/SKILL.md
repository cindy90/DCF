...
SESSION H   — Comps tab (可比公司估值分析)     → Read references/session-h.md
               └─ tushare 拉取可比公司数据 + 5 Block 估值分析
SESSION J   — Valuation_Summary tab            → Read references/session-j.md
               └─ 综合估值汇总 (DCF+Comps 加权) + 估值区间图 + 分析师结论
```
...
Checks:    BS: TA−TLE=0 | CF: TotalCash−BSCash=0 | NI: Model−Source=0 | Rev: Model−Source=0
```
...
- Base-case cell (WACC=11%, g=3%, 10×) highlighted green.
...
- **Block 1** (Assumption Inputs): Bear / Base / Bull columns.
  ...
  - Bear/Bull columns: `=Assumptions!B{row}+({delta})` or `=MAX(0,Assumptions!B{row}−{delta})`.
- **Block 2** (Financial Summary): 5 forecast years × 3 scenarios. Base = live `IS!/CF!/DCF!` cell refs. Bear/Bull = `Base_cell × scale_factor` or `Base_pct_cell ± delta`.
- **Block 3** (Valuation): Base = `DCF!B{row}` direct links. Bear/Bull = UFCF-scaled inline Gordon/Exit formulas using `DCF!B21` (WACC), `DCF!B41` (g), `DCF!B47` (Exit mult), `DCF!B70` (Net Debt) — all referenced, not hardcoded.
- IS/BS/CF tabs are **never modified** by this sheet.
- WACC, g, Exit multiple, Net Debt always referenced from DCF cells, not typed as numbers.
